...
$_debug_start_time = array_sum( explode( ' ', microtime() ) );
...
foreach ( $resources as &$resource ) {
	...
	if(empty($fileData)){
		error_log('Routes File: ' . $resource .' has an error or is empty.');
	}
		...
		if ( file_exists( ABSPATH . $route['methods'][$_SERVER['REQUEST_METHOD']]['file'] ) ) {
			...
			if ( $_output === 1 || !empty( $buffer ) ) {
				...
				if ( is_array( $_output ) || is_object( $_output ) ) {
					...
					$_debug_finish_time = array_sum( explode( ' ', microtime() ) );
					$_debug_total_time = round( ( $_debug_finish_time - $_debug_start_time ), 4 );
					if ( !empty( $route['debug'] ) ) {
						$_output = array_merge( array( '_render_time' => $_debug_total_time, '_version' => VERSION ), (array)$_output );
					}
