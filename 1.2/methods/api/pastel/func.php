<?php
	...
	function upload_sale_pastel($saleGUID) {
		...
		$sh = $db->get_results("select siteguid,excl,vat,tender_total,datetime,pastelid,agent from sh where guid = '".$saleGUID."'");
		foreach ($sh as $row) {
		...
		$repID = $db->get_var("select pastelid from pasteltranslate where siteguid = '".$siteGUID."' and itemguid = '".$userguid."'");
		if (!empty($repID)) {
			$customer_invoice['SalesRepresentativeId'] = $repID;	
		}
	...
	function emailCustomerInvoice($saleGUID) {
		...
		if (empty($siteguid)) { 
			...
			logtodb("uploadsale",'siteguid is empty');
		} 
			...
			if (is_null($productGUID)) {
				...
				if (is_null($productGUID)) {
					...
					if (!empty($item['Category'])) {
						...
						$catguid = $db->get_var("select itemguid from pasteltranslate where siteguid = '".$siteGUID."' and pastelid = '".$catID."'");
						if (!empty($catguid)) {
							$cat = $db->get_var("select cat from categories where guid = '".$catguid."'");
						}
					}
					$product = array(
						"guid" => $newProductGUID,
					...
					$pexcl = $item['PriceExclusive'];
					$pincl = $item['PriceInclusive'];
					logtodb("pastel products","$newProductGUID | $pexcl | $pincl");
				...
				$update = false;
				$product = $db->get_row("select guid,stockcode,descr,cat,cost,sell,vat,virtual,live from products where guid = '".$productGUID."'");
				$cat = $product->cat;
				$catguid = $db->get_var("select guid from categories where companyguid = '".$companyGUID."' and cat = '".$product->cat."'");
				$catpastelid = $db->get_var("select pastelid from pasteltranslate where siteguid = '".$siteGUID."' and itemguid = '".$catguid."'");
				if ($gotCat) {
					$pastelCatID = $item['Category']['ID'];
					if ($pastelCatID != $catpastelid) {
						$catguid = $db->get_var("select itemguid from pasteltranslate where siteguid = '".$siteGUID."' and pastelid = '".$pastelCatID."'");
						$cat = $db->get_var("select cat from categories where companyguid = '".$companyGUID."' and guid = '".$catguid."'");
						if (!empty($cat)) {
							$db->update("products",array('cat' => $cat),array('guid' => $productGUID));
						}
					}
				}
				if ($gotCat == false && !empty($catpastelid)) {
					//update_product_on_pastel($productGUID);
				}
	...
	function update_product_on_pastel($productGUID) {
		...
		foreach ($sites as $site) {
			...
			$cat = $product->cat;
			$catguid = $db->get_var("select guid from categories where companyguid = '".$companyGUID."' and cat = '".$cat."'");
			$catpastelid = $db->get_var("select pastelid from pasteltranslate where siteguid = '".$siteguid."' and itemguid = '".$catguid."'");
			if (!empty($catpastelid)) {
				$category = array(
					'ID' => $catpastelid,
				);
			}
		...
		if (!empty($header)) {
			...
			if (!empty($result['ID'])) {
				if (is_numeric($result['ID'])) {
					...
					logtodb("sendSupplierInvoice",$result);
					logtodb("sendSupplierInvoice",$moveGUID);
		...
		if (!empty($header)) {
			...
			if (!empty($result['ID'])) {
				if (is_numeric($result['ID'])) {
					...
					logtodb("sendSupplierReturn",$result);
					logtodb("sendSupplierReturn",$moveGUID);
				}
			}
		}
	}
	...
	function itemCategory($siteguid) {
		global $db;
		...
		$companyguid = $db->get_var("select coguid from sites where guid = '".$siteguid."'");
		$pastelCategories =  getAllIDs($pastelID,'ItemCategory/Get',array());		
		foreach ($pastelCategories as $cat) {
			$itemguid = $db->get_var("select itemguid from pasteltranslate where siteguid = '".$siteguid."' and pastelid = '".$cat['ID']."'");
			if (empty($itemguid)) {
				$itemguid = $db->get_var("select guid from categories where companyguid = '".$companyguid."' and category = '".$cat['Description']."'");
				if (empty($itemguid)) {
					$guid = gen_uuid();
					$maxcat = $db->get_var("select max(cat)+1 from categories where companyguid = '".$companyguid."'");
					if (empty($maxcat)) { $maxcat = 0; }
					$ins = array(
						'guid'        => $guid,
						'companyguid' => $companyguid,
						'cat'         => $maxcat,
					...
					$db->insert("categories",$ins);
					insertPastelTranslate($siteguid,$guid,$cat['ID']);
				} else {
					insertPastelTranslate($siteguid,$itemguid,$cat['ID']);
				}
			} else {
				$category = $db->get_var("select category from categories where guid = '".$itemguid."'");
				if ($cat['Description'] != $category) {
					$db->update("categories",array('category' => $cat['Description']),array('guid' => $itemguid,'companyguid' => $companyguid));
				}
			}
		}

		$categories = $db->get_results("select * from categories where companyguid = '".$companyguid."' and guid not in (select itemguid from pasteltranslate where siteguid = '".$siteguid."')");
		foreach ($categories as $cat) {
			$ins = array(
				'Description' => $cat->category,
			);
			$result = do_pastel_call('ItemCategory/Save',$p_args,$ins);
			if (!empty($result['ID'])) {
				insertPastelTranslate($siteguid,$cat->guid,$result['ID']);
			} else {
	...
	function salesReps($siteguid) {
		...
		$companyguid = $db->get_var("select coguid from sites where guid = '".$siteguid."'");
		$pastelSalesRepresentatives =  getAllIDs($pastelID,'SalesRepresentative/Get',array());		
		foreach ($pastelSalesRepresentatives as $rep) {
			$ID = $rep['ID'];
			$FirstName = $rep['FirstName'];
			$LastName = $rep['LastName'];
			$Active = $rep['Active'];
			$Email = $rep['Email'];
			if (empty($rep['Mobile'])) { $rep['Mobile'] = ''; }
			$Mobile = $rep['Mobile'];
			$itemguid = $db->get_var("select itemguid from pasteltranslate where siteguid = '".$siteguid."' and pastelid = '".$ID."'");
			if (empty($itemguid)) {
				$guid = $db->get_var("select uguid from users where cguid = '".$companyguid."' and fname = '".$FirstName."' and sname = '".$LastName."'");
				if (empty($guid)) {
					$guid = gen_uuid();
					$ins = array(
						'uguid'      => $guid,
					...
					$db->insert("users",$ins);
					insertPastelTranslate($siteguid,$guid,$ID);
				} else {
					insertPastelTranslate($siteguid,$guid,$ID);
				}
			}
		...
		$users = $db->get_results("select * from users where cguid = '".$companyguid."' and uguid not in (select itemguid from pasteltranslate where siteguid = '".$siteguid."')");
		foreach ($users as $user) {
			$ins = array(
				'FirstName' => $user->fname,
				'LastName'  => $user->sname,
			);
			if ($user->live == 1) { $ins['Active'] = true; } else { $ins['Active'] = false; }
			if (!empty($user->email)) { $ins['Email'] = $user->email; }
			if (!empty($user->cellnr)) { $ins['Mobile'] = $user->cellnr; } else { $ins['Mobile'] = ''; }
			$result = do_pastel_call('SalesRepresentative/Save',$p_args,$ins);
			if (!empty($result['ID'])) {
				insertPastelTranslate($siteguid,$user->uguid,$result['ID']);
			} else {
				console("Error in salesReps");
				console($result);
			}
		}
