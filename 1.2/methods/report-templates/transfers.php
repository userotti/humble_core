...
if(!empty($data['data'])){
	...
	foreach($data['data'] as &$prerow){
		...
		$reference = $prerow['refnr'];
		$processdate = $prerow['datetime'];
		$direction = ucwords(strtolower($prerow['direction']));
		$byuser = $users[$prerow['userguid']]->fname." ".$users[$prerow['userguid']]->sname;
...
<!-- heading -->
<h1 style="font-weight: 400;"><?php echo $report_title . " ". $direction ." - " . $site->sitename . ' - ' . $reference; ?></h1>
<div style="text-align: right;"><?php echo $site->sitename; ?></div>
<div style="text-align: right;">Processed on <?php echo $processdate; ?> by <?php echo $byuser; ?></div>
