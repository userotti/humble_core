...
<!-- heading -->
<h1 style="font-weight: 400;">Adjustment <?php echo $direction; ?> - <?php echo $reference; ?></h1>
		...
		if(!empty($rows)){
			foreach ($rows['lines'] as $index=>&$line) {
					...
					$subtotals = array();
						...
						if(!isset($subtotals['LineCost'])){
							$subtotals['LineCost'] = 0;
						}


						$subtotals['Quantity'] += $line['Quantity'];
						$subtotals['Cost'] += $line['Cost']/$line['Quantity'];
						$subtotals['LineCost'] += $line['Cost'];



						$totals['Quantity'] += $line['Quantity'];
						$totals['Cost'] += $line['Cost']/$line['Quantity'];
						$totals['LineCost'] += $line['Cost'];
					...
					<tr class="midline">
						...
						<td style="text-align: right; width: 80px;"><?php echo money_format('%i', $subtotals['LineCost']); ?></td>
					</tr>
...
?>
	...
	<tfoot>
		<tr>
			...
			<th style="text-align: right; width: 80px;"><?php echo money_format('%i', $totals['LineCost']); ?></th>
		</tr>
	</tfoot>
</table>
</body>
