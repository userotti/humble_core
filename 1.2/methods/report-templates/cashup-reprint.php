...
if(!empty($data['data'])){
	...
	foreach($data['data'] as &$row){
		...
		foreach($rowfields as $field=>$Label){
			if(isset($row[$field])){
				$blocks[$row['cashier']][$index][$field] = $row[$field];
			}else{
				$blocks[$row['cashier']][$index][$field] = null;
			}
		}
...
<!-- heading -->
<h1 style="font-weight: 400;">Cash Up Reprint</h1>
...
<?php 
		...
		if(!empty($rows)){
			...
			<?php foreach($row as $field=>$value){
				...
				// apply difference
				if($field == 'difference'){
					$value = ( $row['declarecash'] + $row['declareccard'] + $row['declaredcard'] + $row['declareacc'] ) - ( $row['cash'] + $row['ccard'] + $row['dcard'] + $row['acc'] );
				}
